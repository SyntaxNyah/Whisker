...
3. **Plugins over forks.** The plugin system lets you add commands, packet hooks, lifecycle hooks, connection filters, moderation actions, area management, status tags, and server-wide broadcasts without modifying core code. The PluginAPI exposes 57 function pointers covering registration, client operations, area operations, broadcasting, packet field access, moderation (including custom-reason kick), argument splitting, ASCII lower-casing, client lifecycle events, connection filtering, and per-client status tags. The extended API (v2) added 15 functions for packet inspection, moderation actions, server-wide broadcasts, area manipulation, player counting, and IPID access; later additions appended `args_split` (the quote-aware argument tokenizer) and `to_lower` (an ASCII lower-caser); a v3 block then appended **lifecycle hooks** (`register_lifecycle_hook` — JOIN/LEAVE/UPDATE events fired from `server::notify_client_*`, the only way a plugin can observe disconnects and state changes a packet hook can't) plus the `client_get_ooc_name` / `client_is_hidden` getters they need; a v4 block then appended **connection filters** (`register_conn_filter` — a predicate the server calls for every inbound connection at accept time, before a handler thread is spawned, returning whether to reject it) plus `client_get_ip` (the raw address, not the hashed IPID) and `broadcast_perm_raw` (a role-gated broadcast); a v5 block then appended a **per-client status tag** (`client_set_status_tag` / `client_get_status_tag`) that the core renders after the player's name in `/ga` and `/gas` (dormant when unset); a v6 block then appended **`client_kick_msg`** (disconnect with a custom KK reason). All of these are appended at the end of the struct for backwards compatibility with existing compiled plugins — the optional player-list plugin is built entirely on the v3 lifecycle hooks, the optional `ip_guard` plugin (IP/CIDR/ASN/country blocking) on the v4 connection filter, the optional `afk` plugin (away indicator) on the v5 status tag, and the optional `lockdown` plugin (known-IPIDs-only switch) on the v6 custom-reason kick. **Plugin command/hook/lifecycle handlers run on the server's per-client threads where the plugin's *own* C3 temp allocator is not initialised, so handlers that use `string::tformat`/`dstring` must either build into fixed buffers (like `player_list`/`afk`) or wrap the handler body in `@pool_init(mem, ...)` (like `casing`/`ip_guard`/`lockdown`).**
